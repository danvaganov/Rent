...
public class ArticleInStockBean extends ArticleBean {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	
	public ArticleInStockBean() {
		super();
	}
	...
	public List<Article> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
		return articleEJB.find(loginBean.gettUser().getEmployee().getOrganization(), ArticleState.IN_STOCK, first, pageSize, filters);
	}
	
	@Override
	public int getRowCount() {
		return articleEJB.getCountInStock(loginBean.gettUser().getEmployee().getOrganization()).intValue();
	}
